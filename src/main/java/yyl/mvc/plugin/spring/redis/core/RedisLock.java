...
public class RedisLock implements Lock {
     ...
     * @param name 锁名称
     */
    protected RedisLock(StringRedisTemplate redisTemplate, RedisPublishSubscribe pubSub, String keyPrefix, String name) {
    ...
    private void lock(boolean interruptibly) throws InterruptedException {
        ...
        // 解锁消息处理
        Subscriber listener = (topic, body) -> {
            if (UNLOCK_MESSAGE.equals(body)) {
                latch.release();
            }
        };

        // 订阅解锁消息
        pubSub.subscribe(name, listener);
        ...
        try {
            ...
            pubSub.unsubscribe(name, listener);
        }
    }
    ...
    private Long tryAcquireCommand(long threadId) {
        try {
            ...
            script.setResultType(Long.class);// 返回类型是Long
            ...
            final Long status = redisTemplate.execute(script, keys, Long.toString(internalLockLeaseTime), getEntryName(threadId));
            return status;
        } catch (Exception e) {
            LOG.error("!", e);
            return 5L;
    ...
    private boolean tryReleaseCommand(long threadId) {
        try {
            ...
            final List<String> keys = Arrays.asList(lockKey, pubSub.getChannelName(name));
            final Long status = redisTemplate.execute(script, keys, Long.toString(internalLockLeaseTime), getEntryName(threadId), UNLOCK_MESSAGE);
            if (status == null) {
